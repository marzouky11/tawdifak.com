  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (isEditing && article) {
        ...
        // Revalidate both the new slug and the original slug (in case it changed)
        // so the article detail page reflects the edit immediately.
        const tagsToRevalidate = article.slug && article.slug !== slug
          ? [`article-${slug}`, `article-${article.slug}`]
          : [`article-${slug}`];
        await revalidateAll(tagsToRevalidate);
